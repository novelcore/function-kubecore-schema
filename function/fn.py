...
# Import SchemaRegistry at module level for testability
try:
...
class KubeCoreContextFunction:
    ...
    def run_function(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        if requested_schemas:
            accessible_schemas = [
                s for s in accessible_schemas if s in requested_schemas
            ]
        ...
        for schema_name in accessible_schemas:
            ...
            if schema_info:
                relationship_path = self.schema_registry.get_relationship_path(
                ...
                if include_full_schemas:
                    available_schema = response["platformContext"]["availableSchemas"][
                        schema_name
                    ]
                    available_schema["schema"] = schema_info.schema
