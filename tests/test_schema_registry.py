...
class TestSchemaRegistry(unittest.TestCase):
    ...
    def test_get_accessible_schemas_for_app(self):
        ...
        for schema in expected:
            self.assertIn(
                schema,
                schemas,
                f"Expected schema {schema} not found in accessible schemas for XApp",
            )
    ...
    def test_get_accessible_schemas_for_kubesystem(self):
        ...
        for schema in expected:
            self.assertIn(
                schema, schemas, f"Expected schema {schema} not found for XKubeSystem"
            )
    ...
    def test_get_accessible_schemas_for_kubenv(self):
        ...
        for schema in expected:
            self.assertIn(
                schema,
                schemas,
                f"Expected schema {schema} not found in accessible schemas for XKubEnv",
            )
    ...
    def test_get_relationship_path_indirect(self):
        ...
        # Since XGitHubProvider is not directly accessible from XApp,
        # should return empty
        ...
        for resource_type, accessible_schemas in PLATFORM_HIERARCHY.items():
            ...
            for schema in accessible_schemas:
                self.assertIn(
                    schema,
                    self.registry.schemas,
                    f"Schema {schema} for {resource_type} not found",
                )
    ...
    def test_schema_structure_validation(self):
        ...
        for resource_type, schema_info in self.registry.schemas.items():
            ...
            self.assertIsNotNone(
                schema_info.api_version, f"Schema {resource_type} missing api_version"
            )
            self.assertIsNotNone(
                schema_info.kind, f"Schema {resource_type} missing kind"
            )
            self.assertIsInstance(
                schema_info.schema, dict, f"Schema {resource_type} schema is not a dict"
            )
            self.assertIsInstance(
            ...
            self.assertIn(
                "type", schema_info.schema, f"Schema {resource_type} missing type field"
            )
            self.assertEqual(
                schema_info.schema["type"],
                "object",
